...
def fill_excel_file(list_revenues_dict, list_expenses_dict):
    ...
    The function returns an ExcelWriter object containing two sheets: "Recettes" and "Dépenses".
    """
    rows_revenues = []
    rows_expenses = []
    ...
    st.success(f"""Le fichier Excel {excel_path} a été crée avec les recettes des {len(list_revenues_dict)} factures 
               et les dépenses des {len(list_expenses_dict)} notes de frais que vous avez fournies.😃🔥""")
    warning_box = st.empty()
    warning_box.warning(f"⚠️ L'IA peut faire des erreurs, pensez à vérifier systématiquement le contenu du fichier Excel.")
    st.download_button(label="Télécharger le fichier Excel", data=excel_buffer, file_name=str(excel_path))
...
st.title("Recettes et dépenses")
revenues = st.file_uploader("Recettes", type=["csv", "xlsx"], accept_multiple_files=True)
expenses = st.file_uploader("Dépenses", type=["pdf", "png", "jpg"], accept_multiple_files=True)
